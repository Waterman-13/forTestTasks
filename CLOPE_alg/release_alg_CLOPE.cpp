...
double r = 1.6;   // коэффициент отталкивания
int t_num = 4000; // количество транзакций
int t_len = 5;    // длина транзакции
...
void AddTransaction(Cluster &C, char *t)
{
  C.size = C.size + t_len;
  C.count = C.count + 1;
...
void RemoveTransaction(Cluster &C, char *t)
{
  C.size = C.size - t_len;
  C.count = C.count - 1;
...
double AddCost(Cluster C, char *t)
{
  int S = C.size;
  int N = C.count;
  int W = C.width;
...
double RemoveCost(Cluster C, char *t)
{
  int S = C.size;
  int N = C.count;
  int W = C.width;
